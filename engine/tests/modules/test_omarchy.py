...
def _isolate_gpu_marker(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Make plan assertions host-independent.

    ``build_plan`` auto-injects the ``hardware-nvidia`` modules (and their ``docker``
    dependency) when the host's ``~/.local/state/devboost/gpu-vendor`` marker names nvidia.
    On an nvidia workstation that would leak ``docker`` into every Omarchy plan and break
    these assertions; point ``XDG_STATE_HOME`` at an empty dir so no marker is found.
    """
