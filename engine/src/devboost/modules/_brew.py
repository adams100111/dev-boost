...
class BrewFormula:
    ...
    installed that brew never installed. ``ctx.force`` upgrades in place: on macOS
    ``devboost install --update`` forces every module whose macOS install is one Homebrew
    formula or cask (``cli.app._brew_managed_on_macos``), not only ``self_updating`` ones.
