#!/usr/bin/env bash
# modules/devops-lsp/verify.sh — idempotency guard for the devops-lsp module.
# GREEN iff every DevOps server resolves via `mise which` AND its lsp.<lang>
# entry is present and enabled in ~/.config/fresh/config.json.
