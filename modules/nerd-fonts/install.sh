...
#   In Ghostty, use font-family = "JetBrainsMono Nerd Font" directly (no Mono suffix).
set -Eeuo pipefail
...
if fc-list | grep -qi 'JetBrainsMono Nerd Font'; then
  ...
  # -------------------------------------------------------------------------
  # Step 2a: download JetBrainsMono Nerd Font (pinned, single zip expanded
  # to individual .ttf files).
  # -------------------------------------------------------------------------
  log_info "nerd-fonts: downloading JetBrainsMono Nerd Font ${_NF_VERSION}"
  curl -fsSL \
    -o "${_FONT_DIR}/JetBrainsMonoNerdFont-Regular.ttf" \
    "${_NF_BASE}/JetBrainsMono.zip" \
    || { log_error "nerd-fonts: failed to download JetBrainsMono"; exit 1; }

  # -------------------------------------------------------------------------
  # Step 2b: download MesloLGS Nerd Font Mono (pinned).
  ...
  curl -fsSL \
    -o "${_FONT_DIR}/MesloLGSNerdFontMono-Regular.ttf" \
    "${_NF_BASE}/Meslo.zip" \
